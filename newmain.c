...
void main(void) {
    ...
    // Configurações cada interrupção do timer é aproximadamente 1ms
...
void __interrupt() isr(void){
    ...
    if (INTCONbits.INT0F && !fi) { // Botão do mediador ele inicia o modo de jogo e começa o timer
        T = 0;
        fi = 1;
        fix = 1;
        INTCON3bits.INT2F = 0;
        ...
        caracter_inicio(1,4);  
        printf("comecem!"); 
    } else if (INTCON3bits.INT1F && fi) { // Botão do primeiro jogador 
        ...
        caracter_inicio(2,0);
        printf("%d ms",T);
    } else if (INTCON3bits.INT2F && fi) { // Botão do segundo jogador 
        T2CONbits.TMR2ON = 0; // Para o timer
        INTCONbits.INT0F = 0; // Reseta as flags
        fi = 0;
        INTCON3bits.INT2F = 0;
